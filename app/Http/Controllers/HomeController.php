...
// End Tag Auth::user()->userType == "Commercial"
        ...
        $unprocessed = DB::table('estimate')
        ->where('work_order', 0)->where('diagnostics', 0)->where('maintenance', 0)
        ->where('update_by', Auth::user()->station_name)
        ->orderBy('created_at', 'DESC')->get();
        


        $completed = DB::table('estimate')
        ->where('update_by', Auth::user()->station_name)->where('work_order', 1)->orWhere('diagnostics', 1)->orWhere('maintenance', 1)
